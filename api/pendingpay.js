...
module.exports = async (req, res) => {
  ...
  if (origin !== "https://agrofruit.pages.dev") return res.status(403).json({ error: "Forbidden" });

  const authHeader = req.headers["x-api-key"];
  if (!authHeader || authHeader !== API_AUTH_KEY) return res.status(401).json({ error: "Unauthorized request" });
  ...
  if (req.method === "POST") {
    ...
    userPaymentRef.once("value", async (snapshot) => {
      ...
      try {
        await db.ref(`users/${userId}/pendingPay/${newKey}`).set(paymentData);
